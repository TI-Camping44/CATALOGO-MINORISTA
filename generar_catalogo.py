...
def main():
    try:
        ...
        print("Extrayendo items de listas de precios (CON RADAR DOBLE)...")
        ...
        salgs_id = next((pl['id'] for pl in pricelists if pl['name_clean'] == 'SALGS'), None)
        categorias_datos = {}
        orden_hojas = [
        ...
        for p in products:
            ...
            if es_arma:
                precio_salgs = mapa_precios_prod.get(prod_id, {}).get(salgs_id)
                if not precio_salgs or precio_salgs <= 0:
                    precio_salgs = mapa_precios_tmpl.get(tmpl_id, {}).get(salgs_id, 0.0)
                if not salgs_id or float(precio_salgs or 0) <= 0: continue
