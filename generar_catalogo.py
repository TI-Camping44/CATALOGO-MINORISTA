...
def main():
    try:
        ...
        print(f"Se encontraron {len(products)} productos. Extrayendo imágenes...")
        ...
        for p in products:
            ...
            # STOCK: solo almacenes publicables.
            # SR44 SÍ suma; se excluyen INDGM, NSE, PESS, LIC e IR.
            ...
            # ASEGURAMOS QUE LAS ARMAS TENGAN PRECIO SALGS
            es_arma = "ARMA" in categoria_str and "ACCESORIO" not in categoria_str
            if es_arma:
                precio_salgs = mapa_precios_prod.get(prod_id, {}).get("SALGS")
                if not precio_salgs or precio_salgs <= 0:
                    precio_salgs = mapa_precios_tmpl.get(tmpl_id, {}).get("SALGS", 0.0)
                if float(precio_salgs or 0) <= 0: continue
            ...
            # AHORA EL PRODUCTO SE VA A SU MARCA SIEMPRE (Ignora si es promo para la caja)
            hoja = "OTRO"
        ...
        for p in categorias_datos["Todo"]:
            ...
            prod_dict = {
                ...
                "h": p['hoja_asignada'],
                "l": p['es_promo'], # promo flag (categoria con PROMO)
                "pr": base_price,
                "p": {}
            }
        ...
        <style>
            ...
            .btn-filtro{color:#081226;font-size:0.9rem;padding:9px 14px;font-weight:600;border-radius:30px;border:1px solid #dee2e6;text-align:left;width:100%;cursor:pointer;background:#fff;transition:0.2s;margin-bottom:4px;}
            .btn-filtro.active{background-color:#081226;color:white;border-color:#081226;}
            .btn-filtro[data-filtro="🔥 PROMO"] { color: #dc3545; border-color: #f8d7da; background-color: #fff5f5; }
            .btn-filtro[data-filtro="🔥 PROMO"].active { background-color: #dc3545; color: white; border-color: #dc3545; }
            ...
            .check-group{display:flex;flex-wrap:wrap;gap:20px;align-items:center;}
            ...
            .custom-check-spacing { margin-right: 15px !important; margin-bottom: 5px !important; }
        ...
        # Generar los botones dinámicos (contando los PROMO al vuelo)
        ...
        <div class='col-12 col-lg-10 py-3'>
            ...
            <div class='pdf-panel shadow-sm mb-3'>
                <div class='check-group'>
                    <span class='pdf-panel-title pe-3'>📄 COTIZACIÓN PDF:</span>
                    <div class='form-check form-check-inline custom-check-spacing'><input class='form-check-input check-tarifa-pdf' type='checkbox' value='SALGS' id='chk_SALGS' checked><label class='form-check-label fw-bold text-dark ms-1' for='chk_SALGS'>SALGS</label></div>
                    <div class='form-check form-check-inline custom-check-spacing zona-seguridad'><input class='form-check-input check-tarifa-pdf' type='checkbox' value='MAYGS' id='chk_MAYGS'><label class='form-check-label fw-bold text-danger ms-1' for='chk_MAYGS'>MAYGS</label></div>
                    <div class='form-check form-check-inline custom-check-spacing zona-seguridad'><input class='form-check-input check-tarifa-pdf' type='checkbox' value='DIST 1' id='chk_DIST1'><label class='form-check-label fw-bold text-danger ms-1' for='chk_DIST1'>DIST 1</label></div>
                    <div class='form-check form-check-inline custom-check-spacing zona-seguridad'><input class='form-check-input check-tarifa-pdf' type='checkbox' value='DIST 2' id='chk_DIST2'><label class='form-check-label fw-bold text-danger ms-1' for='chk_DIST2'>DIST 2</label></div>
                    <div class='form-check form-check-inline custom-check-spacing border-start ps-4'><input class='form-check-input' type='checkbox' id='chkMostrarStock' checked><label class='form-check-label small text-muted fw-bold ms-1' for='chkMostrarStock'>📦 Stock</label></div>
                    <div class='form-check form-check-inline custom-check-spacing border-start ps-4'><input class='form-check-input' type='checkbox' id='chkOcultarAgotados' checked><label class='form-check-label small text-muted fw-bold ms-1' for='chkOcultarAgotados'>🚫 Ocultar Agotados</label></div>
                </div>
        ...
        <script>
            ...
            function renderizarPagina() {
                ...
                items.forEach(p => {
                    ...
                    if (p.l === true && !p.p['SALGS'] && OFERTAS_EXACTAS[p.c]) {
                        p.p['SALGS'] = '-';
                    }
                ...
                setTimeout(async () => {
                    try {
                        ...
                        productosParaPDF.forEach(p => {
                            ...
                            if (p.l === true && !p.p['SALGS'] && OFERTAS_EXACTAS[p.c]) p.p['SALGS'] = '-';
